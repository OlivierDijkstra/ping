<div>
                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                ...
                                <span class="ml-2 text-sm text-gray-300">{{ $permission }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                        ...
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="cursor-pointer ml-6 text-sm text-gray-300 underline focus:outline-none" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            ...
            <div class="mt-4 bg-gray-100 px-4 py-2 rounded font-mono text-sm text-gray-300">
                {{ $plainTextToken }}
            </div>
        </x-slot>
